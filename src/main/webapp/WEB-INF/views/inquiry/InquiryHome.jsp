<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
<!DOCTYPE html>
<html>
<head>
<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">

<meta charset="UTF-8">
<title>Insert title here</title>
</head>
<body>

<div class="container">
  <div class="row">
    <div class="col">
     <img src="../image/mainPO.png"  width="1300px;">
     
    </div>
  </div>
  <br><br><hr><br><br>
  
  <div class="row" style="text-align: center; font-size: 17px;">
   <div class="col">
      <img src="../image/inquiry_home.png" width="110px">
      <br><br>
      <p class="font-monospace">홈페이지</p>
    </div>
    <div class="col">
      <img src="../image/inquiry_tiket.png" width="110px">
      <br><br>
      <p class="font-monospace">할인혜택</p>
    </div>
    <div class="col">
      <img src="../image/inquiry_discount.png" width="110px">
      <br><br>
      <p class="font-monospace">영화예매</p>
    </div>
    <div class="col">
      <img src="../image/inquiry_payment.png" width="110px">
      <br><br>
      <p class="font-monospace">결제수단</p>
    </div>
      <div class="col">
      <img src="../image/inquiry_membership.png" width="110px">
      <br><br>
      <p class="font-monospace">멤버십</p>
    </div>
      <div class="col">
      <img src="../image/inquiry_vip.png" width="110px">
      <br><br>
      <p class="font-monospace">VIP</p>
    </div>
      <div class="col">
      <img src="../image/inquiry_theater.png" width="110px">
      <br><br>
      <p class="font-monospace">극장</p>
    </div>
     <div class="col">
      <img src="../image/inquiry_special.png" width="110px">
      <br><br>
      <p class="font-monospace">특별관</p>
    </div>
  </div>
  
  <br><br><hr><br><br>
  
  <div class="row" style="text-align: center; font-size: 20px;">
    <div class="col-4">
    <a href="/inquiry/faq">
    	<img src="../image/inquiryfaq.png" width="110px">
    	<br><br>
      <p class="font-monospace">FAQ ></p></a>
    </div>
    <div class="col-4">
    <a href="/inquiry/list">
    	<img src="../image/inquirysupport.png" width="110px">
    	<br><br>
      <p class="font-monospace">1:1 문의 ></p></a>
    </div>
    <div class="col-4">
    <a href="/inquiry/lost">
	    <img src="../image/inquirylost.png" width="110px">
	    <br><br>
      <p class="font-monospace">분실물 신고 ></p></a>
    </div>
  </div>
  
  <br><br><hr><br><br>
  
  <div class="row" style="text-align: center; font-size: 20px;">
    <div class="col">
      <p class="font-monospace"><b>고객센터 검색</b></p>
    </div>
  </div>
  
  <div class="row" style="text-align: center; font-size: 20px;">
     <div class="col-4">&nbsp;</div>
    <div class="col-4">
      <input type="search" class="form-control" placeholder="키워드 입력" aria-label="Search" style="text-align: center;">
    </div>
    <div class="col-1">
      <button type="button" style="width: 130px; height: 38px;" class="btn btn-outline-dark"><p class=font-monospace>검색하기</p></button>
       <div class="col-3">&nbsp;</div>
    </div>
     
  </div>
   <br><br><hr><br><br>
   
   <div class="row">
    <div class="col">
     <img src="../image/superson.png"  width="1300px;">
     
    </div>
  </div>
  <br><br><hr><br><br>
</div>

<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-ka7Sk0Gln4gmtz2MlQnikT1wXgYsOg+OMhuP+IlRH9sENBO0LRn5q+8nbTov4+1p" crossorigin="anonymous"></script>
</body>
</html>